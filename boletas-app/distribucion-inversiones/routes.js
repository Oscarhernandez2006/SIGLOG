...
router.get("/rutas", (req, res) => {
  ...
  let q = "SELECT * FROM inv_rutas WHERE activo = 1";
  const params = [];
  if (buscar) {
...
router.post("/rutas", (req, res) => {
  ...
  db.run(
    "INSERT INTO inv_rutas (nombre, recorrido, ciudad, kls_recorridos, tiempo, horas) VALUES (?, ?, ?, ?, ?, ?)",
...
router.put("/rutas/:id", (req, res) => {
  const { id } = req.params;
  const { nombre, recorrido, ciudad, kls_recorridos, tiempo, horas } = req.body || {};
  db.get("SELECT * FROM inv_rutas WHERE id = ?", [id], (err, r) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (!r) return res.status(404).json({ error: "Ruta no encontrada" });
    db.run(
      "UPDATE inv_rutas SET nombre = ?, recorrido = ?, ciudad = ?, kls_recorridos = ?, tiempo = ?, horas = ? WHERE id = ?",
      [
...
router.delete("/rutas/:id", (req, res) => {
  const { id } = req.params;
  db.run("UPDATE inv_rutas SET activo = 0 WHERE id = ?", [id], function (err) {
...
router.get("/productos", (req, res) => {
  ...
  const { buscar } = req.query;
  let query = "SELECT * FROM inv_productos";
  const params = [];
...
router.post("/productos", (req, res) => {
  ...
  db.run(
    "INSERT INTO inv_productos (referencia, nombre, descripcion, categoria, precio, unidad_medida, stock) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
router.put("/productos/:id", (req, res) => {
  ...
  db.get("SELECT * FROM inv_productos WHERE id = ?", [id], (err, prod) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (!prod) return res.status(404).json({ error: "Producto no encontrado" });

    db.run(
      "UPDATE inv_productos SET referencia = ?, nombre = ?, descripcion = ?, categoria = ?, precio = ?, unidad_medida = ?, stock = ?, activo = ? WHERE id = ?",
      [
...
router.delete("/productos/:id", (req, res) => {
  const { id } = req.params;
  db.run("UPDATE inv_productos SET activo = 0 WHERE id = ?", [id], function (err) {
...
router.post("/productos/carga-masiva", upload.single("archivo"), (req, res) => {
  ...
  try {
    ...
    filas.forEach(({ fila, hoja }, i) => {
      ...
      if (referencia) {
        db.get("SELECT id FROM inv_productos WHERE referencia = ?", [referencia], (err, existe) => {
          if (existe) {
            db.run(
              "UPDATE inv_productos SET nombre = ?, categoria = ?, precio = ?, unidad_medida = ?, stock = ?, activo = 1 WHERE id = ?",
...
router.get("/distribuidores", (req, res) => {
  ...
  let query = "SELECT * FROM inv_distribuidores";
  const params = [];
...
router.get("/distribuidores/:id", (req, res) => {
  db.get("SELECT * FROM inv_distribuidores WHERE id = ?", [req.params.id], (err, row) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
...
router.post("/distribuidores", (req, res) => {
  ...
  db.run(
    "INSERT INTO inv_distribuidores (nombre, cedula_nit, telefono, direccion, ciudad, zona, email, observaciones, fecha_registro) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
router.put("/distribuidores/:id", (req, res) => {
  ...
  db.get("SELECT * FROM inv_distribuidores WHERE id = ?", [id], (err, dist) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (!dist) return res.status(404).json({ error: "Distribuidor no encontrado" });

    db.run(
      "UPDATE inv_distribuidores SET nombre=?, cedula_nit=?, telefono=?, direccion=?, ciudad=?, zona=?, email=?, observaciones=?, activo=? WHERE id=?",
      [
        nombre || dist.nombre,
...
router.delete("/distribuidores/:id", (req, res) => {
  db.run("UPDATE inv_distribuidores SET activo = 0 WHERE id = ?", [req.params.id], function (err) {
...
router.get("/ordenes", (req, res) => {
  ...
  let query = `
    SELECT o.*,
      ...
      (SELECT COALESCE(SUM(cantidad), 0) FROM inv_orden_items WHERE orden_id = o.id) AS total_pedido,
      (SELECT COALESCE(SUM(cantidad_entregada), 0) FROM inv_orden_items WHERE orden_id = o.id) AS total_entregado
    FROM inv_ordenes o
    LEFT JOIN inv_clientes_all c
      ON c.activo = 1
     AND (c.codigo_concatenado = o.distribuidor_nombre OR c.nombre = o.distribuidor_nombre)
  ...
  `;
  if (!todas) {
    query += " AND o.id NOT IN (SELECT orden_id FROM inv_asignacion_ordenes)";
  }
  const params = [];
...
router.get("/ordenes/:id", (req, res) => {
  ...
  db.get("SELECT * FROM inv_ordenes WHERE id = ?", [id], (err, orden) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (!orden) return res.status(404).json({ error: "Orden no encontrada" });

    db.all("SELECT * FROM inv_orden_items WHERE orden_id = ?", [id], (err2, items) => {
...
router.post("/ordenes", (req, res) => {
  ...
  db.get(
    "SELECT COALESCE(MAX(CAST(SUBSTR(numero_orden, 4) AS INTEGER)), 0) as ultimo FROM inv_ordenes WHERE numero_orden LIKE 'DA-%'",
    [],
    (err, row) => {
      if (err) return res.status(500).json({ error: "Error del servidor" });
      ...
      db.run(
        "INSERT INTO inv_ordenes (numero_orden, distribuidor_id, distribuidor_nombre, fecha, estado, total, observaciones) VALUES (?, ?, ?, ?, 'PENDIENTE', ?, ?)",
        ...
        function (err) {
          ...
          const stmt = db.prepare(
            "INSERT INTO inv_orden_items (orden_id, producto_nombre, cantidad, precio_unitario, subtotal) VALUES (?, ?, ?, ?, ?)"
          );
...
router.put("/ordenes/:id/estado", (req, res) => {
  ...
  db.run("UPDATE inv_ordenes SET estado = ? WHERE id = ?", [estado, id], function (err) {
...
router.put("/ordenes/:id", (req, res) => {
  ...
  db.get("SELECT * FROM inv_ordenes WHERE id = ?", [id], (err, orden) => {
    ...
    db.run(
      "UPDATE inv_ordenes SET distribuidor_id = ?, distribuidor_nombre = ?, observaciones = ?, total = ? WHERE id = ?",
      [
      ...
      function (err) {
        ...
        db.run("DELETE FROM inv_orden_items WHERE orden_id = ?", [id], (err2) => {
...
router.delete("/ordenes/:id", (req, res) => {
  ...
  db.get("SELECT * FROM inv_ordenes WHERE id = ?", [id], (err, orden) => {
    ...
    db.run("DELETE FROM inv_orden_items WHERE orden_id = ?", [id], (err2) => {
      ...
      db.run("DELETE FROM inv_ordenes WHERE id = ?", [id], function (err3) {
  ...
  db.get("SELECT * FROM usuarios WHERE rol = 'administrador' AND password = ?", [password], async (err, user) => {
    ...
    try {
      const countRow = await new Promise((resolve, reject) => {
        db.get("SELECT COUNT(*) AS n FROM inv_ordenes", [], (e, row) => e ? reject(e) : resolve(row));
      });
      ...
      try {
        // Postgres: limpia en cascada cualquier tabla que referencie inv_ordenes
        await tryRun("TRUNCATE TABLE inv_ordenes RESTART IDENTITY CASCADE");
      } catch (eTrunc) {
        ...
        await tryRun("DELETE FROM inv_asignacion_ordenes");
        await tryRun("DELETE FROM inv_orden_items");
        await tryRun("DELETE FROM inv_ordenes");
      }
      ...
      try {
        await tryRun("DELETE FROM inv_asignacion_ordenes");
        await tryRun("DELETE FROM inv_asignaciones WHERE COALESCE(estado,'ACTIVA') = 'ACTIVA'");
      } catch (eAsig) {
...
router.post("/ordenes/:id/entrega", (req, res) => {
  ...
  db.get("SELECT * FROM inv_ordenes WHERE id = ?", [id], (err, orden) => {
    ...
    db.all("SELECT * FROM inv_orden_items WHERE orden_id = ?", [id], (err2, currentItems) => {
      if (err2) return res.status(500).json({ error: "Error del servidor" });

      const stmt = db.prepare("UPDATE inv_orden_items SET cantidad = ?, cantidad_entregada = ? WHERE id = ? AND orden_id = ?");
      items.forEach((it) => {
      ...
      stmt.finalize(() => {
        db.run("UPDATE inv_ordenes SET estado = 'ENTREGADO', entrega_registrada = 1 WHERE id = ?", [id], function (err3) {
...
router.get("/nivel-servicio", (req, res) => {
  ...
  const queries = {
    global: `
      ...
      FROM inv_ordenes o
      LEFT JOIN inv_orden_items i ON i.orden_id = o.id
      WHERE ${where}
    `,
    porCliente: `
      SELECT
    ...
    porProducto: `
      SELECT
        ...
        COALESCE(SUM(i.cantidad), 0) AS pedido,
        COALESCE(SUM(i.cantidad_entregada), 0) AS entregado
      FROM inv_orden_items i
      INNER JOIN inv_ordenes o ON o.id = i.orden_id
      WHERE ${where}
    ...
    porVehiculo: `
      ...
      INNER JOIN inv_asignacion_ordenes ao ON ao.asignacion_id = a.id
      INNER JOIN inv_ordenes o ON o.id = ao.orden_id
      LEFT JOIN inv_orden_items i ON i.orden_id = o.id
      WHERE ${where}
...
router.post("/ordenes/:id/sin-novedad", (req, res) => {
  const { id } = req.params;
  db.get("SELECT id FROM inv_ordenes WHERE id = ?", [id], (err, orden) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (!orden) return res.status(404).json({ error: "Orden no encontrada" });

    db.run("UPDATE inv_orden_items SET cantidad_entregada = cantidad WHERE orden_id = ?", [id], function (err2) {
      if (err2) return res.status(500).json({ error: "Error al actualizar items" });
      db.run("UPDATE inv_ordenes SET estado = 'SIN NOVEDAD' WHERE id = ?", [id], function (err3) {
        if (err3) return res.status(500).json({ error: "Error al actualizar estado" });
...
router.get("/ordenes-pendientes-entrega", (req, res) => {
  const query = `
    SELECT o.*,
      ...
      (SELECT COALESCE(SUM(cantidad_entregada), 0) FROM inv_orden_items WHERE orden_id = o.id) AS total_entregado
    FROM inv_ordenes o
    INNER JOIN inv_asignacion_ordenes ao ON ao.orden_id = o.id
    INNER JOIN inv_asignaciones a ON a.id = ao.asignacion_id
    WHERE 1=1
...
router.get("/vehiculos", (req, res) => {
  ...
  let query = `SELECT v.*, t.cedula AS tripulacion_cedula,
                 ...
                 FROM inv_vehiculos v
                 LEFT JOIN inv_tripulacion t ON t.id = v.tripulacion_id`;
  const params = [];
...
router.get("/vehiculos/:id", (req, res) => {
  db.get("SELECT * FROM inv_vehiculos WHERE id = ?", [req.params.id], (err, row) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
...
router.post("/vehiculos", (req, res) => {
  ...
  db.run(
    "INSERT INTO inv_vehiculos (placa, conductor, disponibilidad) VALUES (?, ?, ?)",
...
router.put("/vehiculos/:id", (req, res) => {
  ...
  db.get("SELECT * FROM inv_vehiculos WHERE id = ?", [id], (err, veh) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    ...
    const aplicar = (conductorFinal, tidFinal) => {
      ...
      db.run(
        "UPDATE inv_vehiculos SET placa=?, conductor=?, disponibilidad=?, activo=?, tripulacion_id=? WHERE id=?",
        [
    ...
    if (tripulacion_id !== undefined) {
      if (tripulacion_id === null || tripulacion_id === "") {
        ...
        db.get(
          "SELECT id, placa FROM inv_vehiculos WHERE tripulacion_id = ? AND id <> ? AND activo = 1",
          [tripulacion_id, id],
          (eDup, dup) => {
            ...
            db.get("SELECT nombres, apellidos FROM inv_tripulacion WHERE id = ?", [tripulacion_id], (e, t) => {
...
router.delete("/vehiculos/:id", (req, res) => {
  db.run("UPDATE inv_vehiculos SET activo = 0 WHERE id = ?", [req.params.id], function (err) {
...
router.post("/tripulacion", (req, res) => {
  ...
  verificarAdmin(password, (ok) => {
    ...
    db.run(
      `INSERT INTO inv_tripulacion (cedula, nombres, apellidos, telefono, rol, nombre, tipo)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
router.delete("/tripulacion/:id", (req, res) => {
  ...
  verificarAdmin(password, (ok) => {
    if (!ok) return res.status(401).json({ error: "Contraseña de administrador incorrecta" });
    db.run("DELETE FROM inv_tripulacion WHERE id = ?", [req.params.id], function (err) {
...
router.post("/vehiculos/carga-masiva", upload.single("archivo"), (req, res) => {
  ...
  try {
    ...
    filasValidas.forEach((fila, i) => {
      ...
      db.get("SELECT id FROM inv_vehiculos WHERE placa = ?", [placa], (err, existe) => {
        if (existe) {
          db.run("UPDATE inv_vehiculos SET conductor = ?, disponibilidad = ?, activo = 1 WHERE id = ?",
          ...
          db.run("INSERT INTO inv_vehiculos (placa, conductor, disponibilidad) VALUES (?, ?, ?)",
...
// Módulo Inversiones: los clientes viven en la tabla compartida
// tat_clientes_inversiones (la misma del grupo Inversiones de TaT).
...
router.post("/ordenes/verificar-excel", upload.single("archivo"), (req, res) => {
  ...
  try {
    ...
    db.all(
      "SELECT DISTINCT nombre, codigo_concatenado FROM inv_clientes_all WHERE (nombre IN (" + placeholders + ") OR codigo_concatenado IN (" + placeholders + ")) AND activo = 1",
      allPlaceholders,
      (err, rows) => {
        if (err) return res.status(500).json({ error: "Error del servidor" });
...
router.post("/ordenes/carga-excel", upload.single("archivo"), (req, res) => {
  ...
  try {
    ...
    db.all(
      "SELECT DISTINCT nombre, codigo_concatenado FROM inv_clientes_all WHERE activo = 1 AND (codigo_concatenado IN (" + placeholders + ") OR nombre IN (" + placeholders + "))",
      params,
      (errC, rowsC) => {
          ...
          db.run(
            ...
            function (err2) {
              ...
              itemsEntries.forEach(([prodNombre, cant]) => {
                db.run(
                  "INSERT INTO inv_orden_items (orden_id, producto_nombre, cantidad, precio_unitario, subtotal, cantidad_entregada) VALUES (?, ?, ?, 0, 0, 0)",
...
router.get("/asignaciones", (req, res) => {
  ...
  db.all(`SELECT * FROM inv_asignaciones WHERE ${whereEstado} ORDER BY fecha DESC`, [], (err, rows) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    ...
    asignaciones.forEach(a => {
      db.all(
        `SELECT ao.orden_id, o.numero_orden,
                COALESCE(c.nombre, o.distribuidor_nombre) AS distribuidor_nombre,
                o.total
         FROM inv_asignacion_ordenes ao
         LEFT JOIN inv_ordenes o ON o.id = ao.orden_id
         LEFT JOIN inv_clientes_all c
           ON c.activo = 1
          AND (c.codigo_concatenado = o.distribuidor_nombre OR c.nombre = o.distribuidor_nombre)
...
router.post("/asignaciones", (req, res) => {
  ...
  db.get("SELECT * FROM inv_vehiculos WHERE id = ? AND activo = 1", [vehiculo_id], (err, vehiculo) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (!vehiculo) return res.status(404).json({ error: "Vehículo no encontrado" });

    const fecha = new Date().toISOString();
    db.run(
      "INSERT INTO inv_asignaciones (vehiculo_id, vehiculo_placa, vehiculo_conductor, fecha, observaciones) VALUES (?, ?, ?, ?, ?)",
      ...
      function (err2) {
        ...
        orden_ids.forEach(ordenId => {
          db.run(
            "INSERT INTO inv_asignacion_ordenes (asignacion_id, orden_id) VALUES (?, ?)",
...
router.delete("/asignaciones/:id", (req, res) => {
  const { id } = req.params;
  db.run("DELETE FROM inv_asignacion_ordenes WHERE asignacion_id = ?", [id], (err) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    db.run("DELETE FROM inv_asignaciones WHERE id = ?", [id], function (err2) {
      if (err2) return res.status(500).json({ error: "Error del servidor" });
...
// (incluye casos en que inv_asignacion_ordenes apunta a órdenes ya eliminadas)
router.post("/asignaciones/limpiar-vacias", (req, res) => {
  // Primero, borrar referencias huérfanas en inv_asignacion_ordenes
  const limpiarHuerfanas = `
    DELETE FROM inv_asignacion_ordenes
    WHERE orden_id NOT IN (SELECT id FROM inv_ordenes)
  `;
  db.run(limpiarHuerfanas, [], (eH) => {
    ...
    const sql = `
      DELETE FROM inv_asignaciones
      WHERE COALESCE(estado,'ACTIVA') = 'ACTIVA'
        AND id NOT IN (SELECT DISTINCT asignacion_id FROM inv_asignacion_ordenes)
    `;
...
router.delete("/asignaciones/:asignacionId/orden/:ordenId", (req, res) => {
  ...
  db.run("DELETE FROM inv_asignacion_ordenes WHERE asignacion_id = ? AND orden_id = ?", [asignacionId, ordenId], function (err) {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    // Si la asignación queda sin órdenes, eliminarla
    db.get("SELECT COUNT(*) as total FROM inv_asignacion_ordenes WHERE asignacion_id = ?", [asignacionId], (err2, row) => {
      if (!err2 && row && row.total === 0) {
        db.run("DELETE FROM inv_asignaciones WHERE id = ?", [asignacionId]);
      }
...
router.get("/asignaciones/exportar-excel", (req, res) => {
  db.all("SELECT * FROM inv_asignaciones WHERE COALESCE(estado,'ACTIVA') = 'ACTIVA' ORDER BY fecha DESC", [], (err, asignaciones) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
      ...
      db.all(
        ...
        (err2, ordenes) => {
          ...
          if (pendientes === 0) {
            ...
            db.run(
              "UPDATE inv_asignaciones SET estado='EXPORTADA', fecha_exportacion=?, observacion_servicio=COALESCE(observacion_servicio,'Sin Novedad') WHERE id IN (" + placeholders + ")",
              [ahora, ...ids],
              () => {
                // Inicializar observacion_servicio = 'Sin Novedad' en las órdenes exportadas (si no la tienen)
                db.run(
                  "UPDATE inv_ordenes SET observacion_servicio = COALESCE(observacion_servicio,'Sin Novedad') WHERE id IN (SELECT orden_id FROM inv_asignacion_ordenes WHERE asignacion_id IN (" + placeholders + "))",
                  ids,
...
router.get("/asignaciones/historial-grupos", (req, res) => {
  ...
  let q = `
    ...
    FROM inv_asignaciones a
    LEFT JOIN inv_asignacion_ordenes ao ON ao.asignacion_id = a.id
    LEFT JOIN inv_ordenes o ON o.id = ao.orden_id
    WHERE a.estado = 'EXPORTADA'
  `;
  const params = [];
    ...
    if (err) {
      ...
      return db.all(
        `SELECT a.id, a.vehiculo_placa, COALESCE(a.fecha_exportacion, a.fecha) AS fecha_exportacion
         FROM inv_asignaciones a WHERE a.estado = 'EXPORTADA'
...
router.get("/asignaciones/historial/exportar-excel", (req, res) => {
  ...
  db.all(
    "SELECT * FROM inv_asignaciones WHERE estado = 'EXPORTADA' AND COALESCE(fecha_exportacion, fecha) = ? ORDER BY fecha DESC",
    [fecha_exportacion],
    (err, asignaciones) => {
...
router.get("/asignaciones/lote", (req, res) => {
  ...
  if (!fecha_exportacion) return res.status(400).json({ error: "fecha_exportacion es requerida" });
  db.all(
    "SELECT id, vehiculo_placa, vehiculo_conductor, COALESCE(auxiliar,'') AS auxiliar FROM inv_asignaciones WHERE estado = 'EXPORTADA' AND COALESCE(fecha_exportacion, fecha) = ? ORDER BY vehiculo_placa",
    [fecha_exportacion],
    (err, asignaciones) => {
      if (err) return res.status(500).json({ error: "Error del servidor: " + err.message });
...
router.put("/asignaciones/:id/lote", (req, res) => {
  ...
  params.push(id);
  db.run("UPDATE inv_asignaciones SET " + sets.join(", ") + " WHERE id = ?", params, function (err) {
    if (err) return res.status(500).json({ error: "Error del servidor" });
...
router.put("/ordenes/:id/total", (req, res) => {
  ...
  db.run("UPDATE inv_ordenes SET total = ? WHERE id = ?", [total, id], function (err) {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (this.changes === 0) return res.status(404).json({ error: "Orden no encontrada" });
...
router.put("/asignaciones/:id/observacion", (req, res) => {
  ...
  db.run("UPDATE inv_asignaciones SET observacion_servicio = ? WHERE id = ?", [obs, id], function (err) {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (this.changes === 0) return res.status(404).json({ error: "Asignación no encontrada" });
...
router.put("/asignaciones/:id/auxiliar", (req, res) => {
  ...
  db.run("UPDATE inv_asignaciones SET auxiliar = ? WHERE id = ?", [aux, id], function (err) {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (this.changes === 0) return res.status(404).json({ error: "Asignación no encontrada" });
...
router.put("/ordenes/:id/observacion", (req, res) => {
  ...
  const { observacion } = req.body || {};
  const obs = (observacion || "Sin Novedad").toString().trim() || "Sin Novedad";
  db.run("UPDATE inv_ordenes SET observacion_servicio = ? WHERE id = ?", [obs, id], function (err) {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (this.changes === 0) return res.status(404).json({ error: "Orden no encontrada" });
    res.json({ message: "Observación actualizada", observacion: obs });
  });
...
router.put("/ordenes/:id/historial", (req, res) => {
  ...
  if (!sets.length) return res.status(400).json({ error: "Nada que actualizar" });
  params.push(id);
  db.run("UPDATE inv_ordenes SET " + sets.join(", ") + " WHERE id = ?", params, function (err) {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (this.changes === 0) return res.status(404).json({ error: "Orden no encontrada" });
...
function generarExcel(asignaciones, res, fechaArchivo) {
  ...
  db.all(
    "SELECT * FROM inv_clientes_all WHERE (nombre IN (" + placeholders + ") OR codigo_concatenado IN (" + placeholders + ")) AND activo = 1",
    allPlaceholders,
    (err, clientes) => {
      const clienteMap = {};
      (clientes || []).forEach(c => {
...
// Consecutivo global de plantilla D.L — llave primaria única en inv_plantillas_dl
...
router.get("/plantillas/:consecutivo", (req, res) => {
  db.get(`SELECT * FROM inv_plantillas_dl WHERE consecutivo = ?`, [req.params.consecutivo], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
...
router.put("/plantillas/:consecutivo", (req, res) => {
  ...
  db.get(`SELECT ordenes_json FROM inv_plantillas_dl WHERE consecutivo = ?`, [consecutivo], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
    if (!row) return res.status(404).json({ error: "Plantilla no encontrada" });
    let ordsActuales = [];
    ...
    db.run(`UPDATE inv_plantillas_dl SET ${sets.join(", ")} WHERE consecutivo = ?`, params, function (e2) {
...
router.delete("/plantillas/:consecutivo/orden", (req, res) => {
  ...
  db.get(`SELECT ordenes_json FROM inv_plantillas_dl WHERE consecutivo = ?`, [consecutivo], (err, row) => {
    ...
    db.run(
      `UPDATE inv_plantillas_dl SET ordenes_json = ?, total_kilos = ?, total_documentos = ?, editado = 1 WHERE consecutivo = ?`,
      ...
      function (e2) {
        ...
        const liberar = (oid) => db.run("DELETE FROM inv_asignacion_ordenes WHERE orden_id = ?", [oid], () => {
        ...
        if (ordenId != null) {
          ...
          db.get("SELECT id FROM inv_ordenes WHERE numero_orden = ?", [numeroOrden], (e3, r3) => {
