...
router.get("/rutas", (req, res) => {
  ...
  let q = "SELECT * FROM tat_rutas WHERE activo = 1";
  const params = [];
  if (buscar) {
...
router.post("/rutas", (req, res) => {
  ...
  db.run(
    "INSERT INTO tat_rutas (nombre, recorrido, ciudad, kls_recorridos, tiempo, horas) VALUES (?, ?, ?, ?, ?, ?)",
...
router.put("/rutas/:id", (req, res) => {
  const { id } = req.params;
  const { nombre, recorrido, ciudad, kls_recorridos, tiempo, horas } = req.body || {};
  db.get("SELECT * FROM tat_rutas WHERE id = ?", [id], (err, r) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (!r) return res.status(404).json({ error: "Ruta no encontrada" });
    db.run(
      "UPDATE tat_rutas SET nombre = ?, recorrido = ?, ciudad = ?, kls_recorridos = ?, tiempo = ?, horas = ? WHERE id = ?",
      [
...
router.delete("/rutas/:id", (req, res) => {
  const { id } = req.params;
  db.run("UPDATE tat_rutas SET activo = 0 WHERE id = ?", [id], function (err) {
...
router.get("/productos", (req, res) => {
  ...
  const { buscar } = req.query;
  let query = "SELECT * FROM tat_productos";
  const params = [];
...
router.post("/productos", (req, res) => {
  ...
  db.run(
    "INSERT INTO tat_productos (referencia, nombre, descripcion, categoria, precio, unidad_medida, stock) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
router.put("/productos/:id", (req, res) => {
  ...
  db.get("SELECT * FROM tat_productos WHERE id = ?", [id], (err, prod) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (!prod) return res.status(404).json({ error: "Producto no encontrado" });

    db.run(
      "UPDATE tat_productos SET referencia = ?, nombre = ?, descripcion = ?, categoria = ?, precio = ?, unidad_medida = ?, stock = ?, activo = ? WHERE id = ?",
      [
...
router.delete("/productos/:id", (req, res) => {
  const { id } = req.params;
  db.run("UPDATE tat_productos SET activo = 0 WHERE id = ?", [id], function (err) {
...
router.post("/productos/carga-masiva", upload.single("archivo"), (req, res) => {
  ...
  try {
    ...
    filas.forEach(({ fila, hoja }, i) => {
      ...
      if (referencia) {
        db.get("SELECT id FROM tat_productos WHERE referencia = ?", [referencia], (err, existe) => {
          if (existe) {
            db.run(
              "UPDATE tat_productos SET nombre = ?, categoria = ?, precio = ?, unidad_medida = ?, stock = ?, activo = 1 WHERE id = ?",
...
router.get("/distribuidores", (req, res) => {
  ...
  let query = "SELECT * FROM tat_distribuidores";
  const params = [];
...
router.get("/distribuidores/:id", (req, res) => {
  db.get("SELECT * FROM tat_distribuidores WHERE id = ?", [req.params.id], (err, row) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
...
router.post("/distribuidores", (req, res) => {
  ...
  db.run(
    "INSERT INTO tat_distribuidores (nombre, cedula_nit, telefono, direccion, ciudad, zona, email, observaciones, fecha_registro) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
router.put("/distribuidores/:id", (req, res) => {
  ...
  db.get("SELECT * FROM tat_distribuidores WHERE id = ?", [id], (err, dist) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (!dist) return res.status(404).json({ error: "Distribuidor no encontrado" });

    db.run(
      "UPDATE tat_distribuidores SET nombre=?, cedula_nit=?, telefono=?, direccion=?, ciudad=?, zona=?, email=?, observaciones=?, activo=? WHERE id=?",
      [
        nombre || dist.nombre,
...
router.delete("/distribuidores/:id", (req, res) => {
  db.run("UPDATE tat_distribuidores SET activo = 0 WHERE id = ?", [req.params.id], function (err) {
...
router.get("/ordenes", (req, res) => {
  ...
  `;
  if (!todas) {
    query += " AND o.id NOT IN (SELECT orden_id FROM tat_asignacion_ordenes)";
  }
  const params = [];
...
router.get("/ordenes/:id", (req, res) => {
  ...
  db.get("SELECT * FROM tat_ordenes WHERE id = ?", [id], (err, orden) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (!orden) return res.status(404).json({ error: "Orden no encontrada" });

    db.all("SELECT * FROM tat_orden_items WHERE orden_id = ?", [id], (err2, items) => {
...
router.post("/ordenes", (req, res) => {
  ...
  db.get(
    ...
    (err, row) => {
      ...
      db.run(
        "INSERT INTO tat_ordenes (numero_orden, distribuidor_id, distribuidor_nombre, fecha, estado, total, observaciones) VALUES (?, ?, ?, ?, 'PENDIENTE', ?, ?)",
        ...
        function (err) {
          ...
          const stmt = db.prepare(
            "INSERT INTO tat_orden_items (orden_id, producto_nombre, cantidad, precio_unitario, subtotal) VALUES (?, ?, ?, ?, ?)"
          );
...
router.put("/ordenes/:id", (req, res) => {
  ...
  db.get("SELECT * FROM tat_ordenes WHERE id = ?", [id], (err, orden) => {
    ...
    db.run(
      "UPDATE tat_ordenes SET distribuidor_id = ?, distribuidor_nombre = ?, observaciones = ?, total = ? WHERE id = ?",
      [
      ...
      function (err) {
        ...
        db.run("DELETE FROM tat_orden_items WHERE orden_id = ?", [id], (err2) => {
...
router.delete("/ordenes/:id", (req, res) => {
  ...
  db.get("SELECT * FROM tat_ordenes WHERE id = ?", [id], (err, orden) => {
    ...
    db.run("DELETE FROM tat_orden_items WHERE orden_id = ?", [id], (err2) => {
      ...
      db.run("DELETE FROM tat_ordenes WHERE id = ?", [id], function (err3) {
  ...
  db.get("SELECT * FROM usuarios WHERE rol = 'administrador' AND password = ?", [password], async (err, user) => {
    ...
    try {
      const countRow = await new Promise((resolve, reject) => {
        db.get("SELECT COUNT(*) AS n FROM tat_ordenes", [], (e, row) => e ? reject(e) : resolve(row));
      });
      ...
      try {
        // Postgres: limpia en cascada cualquier tabla que referencie tat_ordenes
      ...
      try {
        await tryRun("DELETE FROM tat_asignacion_ordenes");
        await tryRun("DELETE FROM tat_asignaciones WHERE COALESCE(estado,'ACTIVA') = 'ACTIVA'");
      } catch (eAsig) {
...
router.post("/ordenes/:id/entrega", (req, res) => {
  ...
  db.get("SELECT * FROM tat_ordenes WHERE id = ?", [id], (err, orden) => {
    ...
    db.all("SELECT * FROM tat_orden_items WHERE orden_id = ?", [id], (err2, currentItems) => {
      if (err2) return res.status(500).json({ error: "Error del servidor" });

      const stmt = db.prepare("UPDATE tat_orden_items SET cantidad = ?, cantidad_entregada = ? WHERE id = ? AND orden_id = ?");
      items.forEach((it) => {
      ...
      stmt.finalize(() => {
        db.run("UPDATE tat_ordenes SET estado = 'ENTREGADO', entrega_registrada = 1 WHERE id = ?", [id], function (err3) {
...
router.get("/nivel-servicio", (req, res) => {
  ...
  const queries = {
    global: `
      ...
      FROM tat_ordenes o
      LEFT JOIN tat_orden_items i ON i.orden_id = o.id
      WHERE ${where}
    `,
    porCliente: `
      SELECT
    ...
    porProducto: `
      SELECT
        ...
        COALESCE(SUM(i.cantidad), 0) AS pedido,
        COALESCE(SUM(i.cantidad_entregada), 0) AS entregado
      FROM tat_orden_items i
      INNER JOIN tat_ordenes o ON o.id = i.orden_id
      WHERE ${where}
...
router.post("/ordenes/:id/sin-novedad", (req, res) => {
  const { id } = req.params;
  db.get("SELECT id FROM tat_ordenes WHERE id = ?", [id], (err, orden) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (!orden) return res.status(404).json({ error: "Orden no encontrada" });

    db.run("UPDATE tat_orden_items SET cantidad_entregada = cantidad WHERE orden_id = ?", [id], function (err2) {
      if (err2) return res.status(500).json({ error: "Error al actualizar items" });
      db.run("UPDATE tat_ordenes SET estado = 'SIN NOVEDAD' WHERE id = ?", [id], function (err3) {
        if (err3) return res.status(500).json({ error: "Error al actualizar estado" });
...
router.get("/ordenes-pendientes-entrega", (req, res) => {
  const query = `
    SELECT o.*,
      ...
      (SELECT COALESCE(SUM(cantidad_entregada), 0) FROM tat_orden_items WHERE orden_id = o.id) AS total_entregado
    FROM tat_ordenes o
    INNER JOIN tat_asignacion_ordenes ao ON ao.orden_id = o.id
    INNER JOIN tat_asignaciones a ON a.id = ao.asignacion_id
    WHERE 1=1
...
router.get("/vehiculos", (req, res) => {
  ...
  let query = `SELECT v.*, t.cedula AS tripulacion_cedula,
                 ...
                 FROM tat_vehiculos v
                 LEFT JOIN tat_tripulacion t ON t.id = v.tripulacion_id`;
  const params = [];
...
router.put("/vehiculos/:id", (req, res) => {
  ...
  db.get("SELECT * FROM tat_vehiculos WHERE id = ?", [id], (err, veh) => {
    ...
    const aplicar = (conductorFinal, tidFinal) => {
      ...
      db.run(
        "UPDATE tat_vehiculos SET placa=?, conductor=?, disponibilidad=?, activo=?, tripulacion_id=? WHERE id=?",
        [
...
router.post("/tripulacion", (req, res) => {
  ...
  verificarAdmin(password, (ok) => {
    ...
    db.run(
      `INSERT INTO tat_tripulacion (cedula, nombres, apellidos, telefono, rol, nombre, tipo)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
router.post("/vehiculos/carga-masiva", upload.single("archivo"), (req, res) => {
  ...
  try {
    ...
    filasValidas.forEach((fila, i) => {
      ...
      db.get("SELECT id FROM tat_vehiculos WHERE placa = ?", [placa], (err, existe) => {
        if (existe) {
          db.run("UPDATE tat_vehiculos SET conductor = ?, disponibilidad = ?, activo = 1 WHERE id = ?",
          ...
          db.run("INSERT INTO tat_vehiculos (placa, conductor, disponibilidad) VALUES (?, ?, ?)",
          ...
          db.run(
            ...
            function (err2) {
              ...
              itemsEntries.forEach(([prodNombre, cant]) => {
                db.run(
                  "INSERT INTO tat_orden_items (orden_id, producto_nombre, cantidad, precio_unitario, subtotal, cantidad_entregada) VALUES (?, ?, ?, 0, 0, 0)",
...
router.get("/asignaciones", (req, res) => {
  ...
  db.all(`SELECT * FROM tat_asignaciones WHERE ${whereEstado} ORDER BY fecha DESC`, [], (err, rows) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    ...
    db.run(
      ...
      function (err2) {
        ...
        orden_ids.forEach(ordenId => {
          db.run(
            "INSERT INTO tat_asignacion_ordenes (asignacion_id, orden_id) VALUES (?, ?)",
...
router.post("/asignaciones/limpiar-vacias", (req, res) => {
  ...
  db.run(limpiarHuerfanas, [], (eH) => {
    ...
    const sql = `
      DELETE FROM tat_asignaciones
      WHERE COALESCE(estado,'ACTIVA') = 'ACTIVA'
        AND id NOT IN (SELECT DISTINCT asignacion_id FROM tat_asignacion_ordenes)
    `;
...
router.get("/asignaciones/exportar-excel", (req, res) => {
  db.all("SELECT * FROM tat_asignaciones WHERE COALESCE(estado,'ACTIVA') = 'ACTIVA' ORDER BY fecha DESC", [], (err, asignaciones) => {
    if (err) return res.status(500).json({ error: "Error del servidor" });
...
router.get("/asignaciones/historial-grupos", (req, res) => {
  ...
  let q = `
    ...
    FROM tat_asignaciones a
    LEFT JOIN tat_asignacion_ordenes ao ON ao.asignacion_id = a.id
    LEFT JOIN tat_ordenes o ON o.id = ao.orden_id
    WHERE a.estado = 'EXPORTADA'
  `;
  const params = [];
...
router.get("/asignaciones/historial/exportar-excel", (req, res) => {
  ...
  db.all(
    "SELECT * FROM tat_asignaciones WHERE estado = 'EXPORTADA' AND COALESCE(fecha_exportacion, fecha) = ? ORDER BY fecha DESC",
    [fecha_exportacion],
    (err, asignaciones) => {
...
router.get("/asignaciones/lote", (req, res) => {
  ...
  if (!fecha_exportacion) return res.status(400).json({ error: "fecha_exportacion es requerida" });
  db.all(
    "SELECT id, vehiculo_placa, vehiculo_conductor, COALESCE(auxiliar,'') AS auxiliar FROM tat_asignaciones WHERE estado = 'EXPORTADA' AND COALESCE(fecha_exportacion, fecha) = ? ORDER BY vehiculo_placa",
    [fecha_exportacion],
    (err, asignaciones) => {
      if (err) return res.status(500).json({ error: "Error del servidor: " + err.message });
...
router.put("/ordenes/:id/historial", (req, res) => {
  ...
  if (!sets.length) return res.status(400).json({ error: "Nada que actualizar" });
  params.push(id);
  db.run("UPDATE tat_ordenes SET " + sets.join(", ") + " WHERE id = ?", params, function (err) {
    if (err) return res.status(500).json({ error: "Error del servidor" });
    if (this.changes === 0) return res.status(404).json({ error: "Orden no encontrada" });
...
router.get("/plantillas/:consecutivo", (req, res) => {
  db.get(`SELECT * FROM tat_plantillas_dl WHERE consecutivo = ?`, [req.params.consecutivo], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
...
router.put("/plantillas/:consecutivo", (req, res) => {
  ...
  db.get(`SELECT ordenes_json FROM tat_plantillas_dl WHERE consecutivo = ?`, [consecutivo], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
    if (!row) return res.status(404).json({ error: "Plantilla no encontrada" });
    let ordsActuales = [];
    ...
    db.run(`UPDATE tat_plantillas_dl SET ${sets.join(", ")} WHERE consecutivo = ?`, params, function (e2) {
...
router.delete("/plantillas/:consecutivo/orden", (req, res) => {
  ...
  db.get(`SELECT ordenes_json FROM tat_plantillas_dl WHERE consecutivo = ?`, [consecutivo], (err, row) => {
    ...
    db.run(
      `UPDATE tat_plantillas_dl SET ordenes_json = ?, total_kilos = ?, total_documentos = ?, editado = 1 WHERE consecutivo = ?`,
      ...
      function (e2) {
        ...
        if (ordenId != null) {
          ...
          db.get("SELECT id FROM tat_ordenes WHERE numero_orden = ?", [numeroOrden], (e3, r3) => {
